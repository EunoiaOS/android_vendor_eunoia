...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Eunoia model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function eunoiaremote()
{
    ...
    local EUNOIA=$(git config --get review.review.lineageos.org.username)
    if [ -z "$EUNOIA_USER" ]
    then
...
function githubremote()
{
    ...
    git remote add github https://github.com/EunoiaOS/$PROJECT
