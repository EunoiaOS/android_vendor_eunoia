...
// Copyright (C) 2018,2021 The EunoiaOS Project
//
...
func init() {
	android.RegisterModuleType("eunoia_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = eunoiaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = eunoiaExpandVariables(ctx, dep_file)
	...
	cmd := eunoiaExpandVariables(ctx, String(g.properties.Cmd))
